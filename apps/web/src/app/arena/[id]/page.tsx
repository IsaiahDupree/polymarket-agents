...
export default async function ArenaAgentDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (sig.kind === "hold") {
      liveDecision = { kind: "hold" };
    } else if (sig.kind === "entry") {
      liveDecision = { kind: "entry", side: sig.side, market_id: sig.market_id, size_usd: sig.size_usd, rationale: sig.rationale };
    } else if (sig.kind === "multi-leg") {
      // For the agent detail page, surface the first leg as the representative
      // entry. The full leg list lives on the arena tick events; this page
      // just needs to show what the strategy is doing right now.
      const first = sig.legs[0];
      liveDecision = first
        ? { kind: "entry", side: first.side, market_id: first.market_id, size_usd: first.size_usd, rationale: `${sig.rationale} · ${sig.legs.length} legs (showing leg 1)` }
        : { kind: "hold" };
    } else {
      liveDecision = { kind: "exit", market_id: sig.market_id, rationale: sig.rationale };
    }
